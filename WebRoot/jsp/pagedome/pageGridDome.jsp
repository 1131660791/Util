...
<script type="text/javascript" src="<%=path%>/js/lib/jquery-1.11.1.js"></script>
 ...
 <!-- 加入分页-->
<link href="<%=path%>/jsp/pagedome/pager/pagination.css" type="text/css"  rel="stylesheet"   />
 <!-- 加入分页-->
<script type="text/javascript" src="<%=path%>/jsp/pagedome/pager/jquery.pagination.js"></script>
...
load_grid = function(state){
      ...
      $.ajax({
		    ...
		    url: <%=path%>+'/PageObjectDome/forwardPageGridDome.action', // 这里只是 举列  并不能真正 跳转
...
<script type="text/javascript">
	//分页加载的主方法
	$(function(){
		var pager = eval({"showCount":10,"totalPage":0,"totalResult":11,"currentPage":2,"currentResult":0}); //分页信息json字符串 ${page} 为后台返回的 分页对象
